...
export function usePolylineEdge(edge: EdgeData, nodes: NodeData[]): UsePolylineEdgeResult {
  ...
  if (!storeEdge) {
    return { points: [], color, isSelected, labelX, labelY, labelFontSize, labelWidth, labelHeight, storeEdge, fromNodeId, toNodeId, to, from, toAnchor, fromAnchor, edgeWidth, arrowheadDimensions };
  }
  ...
  if (allPoints.length >= 2 && storeEdge.label) {
    ...
    labelWidth = Math.max(textLength * charWidth + (labelFontSize * 1.6), labelFontSize * 2);

...
export const createElbowPath = (
  ...
  const getStub = (p: position, side: string) => {
    switch (side) {
      case 'top': return { x: p.x, y: p.y - STUB_LENGTH };
      case 'bottom': return { x: p.x, y: p.y + STUB_LENGTH };
      case 'left': return { x: p.x - STUB_LENGTH, y: p.y };
      case 'right': return { x: p.x + STUB_LENGTH, y: p.y };
      default: return { x: p.x, y: p.y + STUB_LENGTH };
    }
  };
  ...
  if (isVerticalStart === isVerticalEnd) {
    if (isVerticalStart) {
      ...
      if (startSide === endSide) {
        ...
        const channelY = startSide === 'bottom' 
...
export const refreshConnectedEdges = (
  ...
  return currentEdges.map((edge) => {
    ...
    if (typeof edge.from === "string") {
      ...
      if (node) startPos = getAnchorPoint(node, edge.fromAnchor || { side: "bottom" });
    } else {
      startPos = edge.from;
    }

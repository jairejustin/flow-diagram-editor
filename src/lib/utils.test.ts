  ...
  describe("getArrowheadDimensions", () => {
    it("calculates correct dimensions for various edge widths", () => {
      const testCases = [
        ...
        {
          edgeWidth: 1,
          expected: { width: 11, height: 11, refX: 10.5, refY: 5.5 },
        },
        // width = 2 -> size 12
        {
          edgeWidth: 2,
          expected: { width: 12, height: 12, refX: 11.5, refY: 6 },
        },
        // width = 5 -> size 15 (12 + (5-2))
        {
          edgeWidth: 5,
          expected: { width: 15, height: 15, refX: 14.5, refY: 7.5 },
        },
        // width = 10 -> size 20 (12 + (10-2))
        {
          edgeWidth: 10,
          expected: { width: 20, height: 20, refX: 19.5, refY: 10 },
        },
      ];
    ...
    describe("Anchor Point Utility", () => {
      ...
      describe("getAnchorPoint - Ellipse Shape", () => {
        ...
        it("calculates all anchor points for ellipse", () => {
          ...
          expect(getAnchorPoint(node, { side: "top" })).toEqual({
            x: 52,
            y: 0,
          });
          // expect(getAnchorPoint(node, { side: "top" })).toEqual({ x: 50, y: 0 });

          expect(getAnchorPoint(node, { side: "bottom" })).toEqual({
            x: 52,
            y: 50,
          });
          // expect(getAnchorPoint(node, { side: "bottom" })).toEqual({ x: 50, y: 50 });

          expect(getAnchorPoint(node, { side: "left" })).toEqual({
            x: 0,
            y: 27,
          });
          // expect(getAnchorPoint(node, { side: "left" })).toEqual({ x: 0, y: 25 });

          expect(getAnchorPoint(node, { side: "right" })).toEqual({
            x: 100,
            y: 27,
          });
      ...
      describe("getAnchorPoint - Edge Cases", () => {
        it("handles different node sizes", () => {
          ...
          expect(getAnchorPoint(smallNode, { side: "top" })).toEqual({
            x: 27,
            y: 0,
          });
          ...
          expect(getAnchorPoint(largeNode, { side: "top" })).toEqual({
            x: 152,
            y: 0,
          });
        ...
        it("handles node with style properties", () => {
          ...
          expect(getAnchorPoint(styledNode, { side: "top" })).toEqual({
            x: 52,
            y: 0,
          });
